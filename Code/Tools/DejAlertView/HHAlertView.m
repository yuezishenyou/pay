...
- (instancetype)initWithTitle:(NSString *)title message:(NSString *)message cancel:(NSString*)cancel sure:(NSString *)sure handle:(actionHandle)handle; //sureBtn:(NSString *)sureTitle cancleBtn:(NSString *)cancleTitle
{
    if (self = [super init])
    {
        _handle = handle;
        [self initSubViews:title message:message sureBtn:sure cancleBtn:cancel];
    }
    return self;
...
{
    return [[HHAlertView alloc]initWithTitle:title message:message cancel:cancel sure:sure handle:handle];
}




- (void)initSubViews:(NSString *)title message:(NSString *)message sureBtn:(NSString *)sureTitle cancleBtn:(NSString *)cancleTitle
{
    ...
    if (title)
    {
        self.titleLbl = [self GetAdaptiveLable:CGRectMake(2*XLSpace, 2*XLSpace, AlertW-4*XLSpace, 20)
                                       AndText:title
                                    andIsTitle:YES];
    ...
    if (message)
    {
        ...
        self.msgLbl = [self GetAdaptiveLable:CGRectMake(XLSpace, CGRectGetMaxY(self.titleLbl.frame)+XLSpace, AlertW-2*XLSpace, 20) AndText:message
                                  andIsTitle:NO];
